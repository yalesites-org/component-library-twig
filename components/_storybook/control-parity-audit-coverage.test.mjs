 ...
 *
 * THIS FILE IS THE EXPIRING HALF OF A PAIR. The permanent half is
 * `storybook-presence-coverage.test.mjs`, which checks the audit's *no Storybook presence*
 * table and has no expiry. The two were one file until 2026-09-21 and could not stay that
 * way: this half must delete itself on ratification, the other half must outlive the
 * document, and a single file cannot do both -- restoring it with the column filled turned
 * CI red, which is the coupling that forced the split
 ...
 * document is still being completed; it is not a permanent invariant. Deleting it does not
 * take the presence check with it -- that is the whole point of the split.
 *
 ...
 * stale guard pinning a finished document. The audit's own `## What happens next` records
 * the same expiry from the document side.
 *
