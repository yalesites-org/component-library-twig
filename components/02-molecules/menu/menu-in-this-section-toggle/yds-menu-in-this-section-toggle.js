Drupal.behaviors.menuToggle = {
  attach(context) {
    // Selectors.
    const menuToggle = context.querySelector('.secondary-menu-toggle');
    const section = context.querySelector('.in-this-section');
    const sectionOverlay = context.querySelector('.in-this-section__overlay');
    ...
    function toggleMenuState(target, attribute) {
      ...
      if (newMenuState === 'open') {
        ...
        // Set mobile section height for expanded menu sizing.
        body.style.setProperty(
          '--section-height-mobile',
          `${section.offsetHeight + section.getBoundingClientRect().top}px`,
        );
      } else {
    ...
    if (menuToggle) {
      menuToggle.addEventListener('click', () => {
        toggleMenuState(section, mainMenuState);
        trapKeyboard(section);
      });

      window.addEventListener('resize', () => {
        if (getMenuState(section, mainMenuState) === 'closed') {
          section.setAttribute(mainMenuState, 'loaded');
        }
      });
    }
    ...
    document.addEventListener('keyup', (e) => {
      if (e.key === 'Escape') {
        if (section.getAttribute(mainMenuState) === 'open') {
    ...
    if (sectionOverlay) {
      sectionOverlay.addEventListener('click', () => {
        toggleMenuState(section, mainMenuState);
      });
    }
  },
