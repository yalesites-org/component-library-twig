...
Reference them by URL, the same way `images/patterns/wavy.png` is referenced:

```yaml
image__src: '/images/placeholders/placeholder-3x2.png'
```

That path resolves because `webpack/plugins.js` copies `images/placeholders` into
`dist/images/placeholders`, and Storybook serves `dist` as a static directory — see the
comment on `staticDirs` in `.storybook/main.js`.
...
**The committed files are post-imagemin.** `ImageminPlugin` in `webpack/plugins.js` globs
`images/**/*.{png,jpg,gif,svg}` and writes its output back over the *source* files whenever
`NODE_ENV=production`, so a release build re-optimises them in place — as it does every
`images/icons/*.svg` and `images/patterns/wavy.png`. What is committed here is the result of
that pass, which makes it a no-op: a `NODE_ENV=production` build leaves these seven files
byte-identical and cannot dirty the working tree. So after re-running the command above,
run `NODE_ENV=production npm run build` once before committing, or the bytes will differ
